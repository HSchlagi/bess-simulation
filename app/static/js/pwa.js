...
class BESSPWA {
    ...
    async setupPushNotifications() {
        if ('Notification' in window && 'serviceWorker' in navigator) {
            // Berechtigung anfordern (nur wenn noch nicht entschieden)
            if (Notification.permission === 'default') {
                try {
                    const permission = await Notification.requestPermission();
                    console.log('🔔 Notification Berechtigung:', permission);
                    
                    // Wenn Berechtigung verweigert wurde, nicht weiter versuchen
                    if (permission === 'denied') {
                        console.log('ℹ️ Push-Notifications wurden vom Benutzer deaktiviert');
                        return;
                    }
                } catch (error) {
                    // Fehler beim Anfordern der Berechtigung - nicht kritisch
                    console.log('ℹ️ Notification-Berechtigung konnte nicht angefordert werden');
                    return;
                }
            } else if (Notification.permission === 'denied') {
                // Berechtigung bereits verweigert - nicht weiter versuchen
                console.log('ℹ️ Push-Notifications sind deaktiviert');
                return;
            }

            // Push Manager (nur wenn Berechtigung erteilt wurde)
            if (this.registration && 'pushManager' in this.registration && Notification.permission === 'granted') {
                try {
                    ...
                    // Fehler bei Push-Subscription - nicht kritisch
                    if (error.name === 'NotAllowedError' || error.message.includes('denied') || error.message.includes('permission')) {
                        console.log('ℹ️ Push-Notifications: Berechtigung verweigert');
                    } else {
                        console.log('ℹ️ Push-Subscription nicht verfügbar');
                    }
                }
            }
        }
    ...
    async subscribeToPush() {
        // Prüfe zuerst, ob Berechtigung erteilt wurde
        if (Notification.permission !== 'granted') {
            console.log('ℹ️ Push-Notifications: Berechtigung nicht erteilt');
            return;
        }
        ...
        try {
            ...
            if (error.name === 'NotAllowedError' || error.message.includes('denied') || error.message.includes('permission')) {
                ...
                console.log('ℹ️ Push-Subscription nicht verfügbar:', error.message);
            }
        }
    }
